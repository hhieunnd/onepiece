...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpJump:
			...
			vm.currentFrame().ip = pos - 1 // End of each loop, will inc ip + 1, so don't worry about it
		case code.OpJumpNotTruthy:
			pos := int(code.ReadUint16(ins[ip+1:]))
			vm.currentFrame().ip += 2 // 2 address of jump
...
func (vm *VM) executeHashIndex(left, index object.Object) error {
	...
	if !ok {
		return fmt.Errorf("unsuale as hash key: %s", key)
	}
